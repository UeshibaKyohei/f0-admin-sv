	...
	<div class="overflow-y-auto flex-1 py-3 px-3">
		{#each menuData as section}
			<div class="px-2 {section.title !== menuData[0].title ? 'mt-6' : ''} mb-1">
				<h3 class="text-xs font-semibold text-base-content/50 uppercase tracking-wider">{section.title}</h3>
			</div>
					...
					{#each section.items as item}
						<li>
							{#if item.type === 'link' && item.path}
								<a href={item.path} class="sidebar-link group" class:active={isMenuActive(item.path)}>
							...
							{:else if item.type === 'submenu' && item.path}
								<button 
									...
									class="sidebar-link group w-full text-left flex justify-between items-center {isParentMenuActive(item.path) ? 'active' : ''}"
								>
									<div class="flex items-center gap-3">
										<svg 
									...
									<svg 
										xmlns="http://www.w3.org/2000/svg" 
										class="h-4 w-4 transition-transform duration-200 {isSubmenuExpanded(item.path) ? 'rotate-180' : ''}" 
										fill="none" 
										viewBox="0 0 24 24" 
										stroke="currentColor"
									>
									...
									<ul class="mt-1 ml-4 pl-4 border-l border-base-200/50 space-y-1">
										{#each item.children as child}
											<li>
												<a 
													href={child.path || '#'} 
													class="sidebar-link text-sm group {child.path ? (isMenuActive(child.path) ? 'active' : '') : ''}"
												>
