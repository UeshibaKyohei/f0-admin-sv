...
<div class="space-y-6">
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    ...
    <div class="flex gap-2 items-center">
      <span class="text-sm text-base-content/70">期間:</span>
              ...
              <div class="flex justify-center gap-6 mt-2">
                {#each monthlySales.datasets as dataset, i}
                  <div class="flex items-center gap-2">
                    <div class={`w-3 h-3 rounded-full ${i === 0 ? 'bg-primary' : 'bg-secondary'}`}></div>
          ...
          {#each monthlySales.labels as label, i}
            <div class="text-center">
              ...
              <div class="flex flex-col items-center gap-1 mt-1">
                <span class="text-xs text-primary">{monthlySales.datasets[0].data[i]}M</span>
                <span class="text-xs text-secondary">{monthlySales.datasets[1].data[i]}M</span>
              </div>
            </div>
          {/each}
        </div>
        ...
        <div class="space-y-4">
          {#each regionSales as item}
            <div class="space-y-1">
              ...
              <div class="text-right text-xs text-base-content/60">{item.percentage}%</div>
            </div>
          {/each}
        </div>
      </div>
        ...
        <div class="space-y-4">
          {#each categorySales as item}
            <div class="space-y-1">
              ...
              <div class="w-full bg-base-200 rounded-full h-2.5">
                <div class="bg-secondary h-2.5 rounded-full" style="width: {item.percentage}%"></div>
              </div>
              <div class="text-right text-xs text-base-content/60">{item.percentage}%</div>
            </div>
          {/each}
        ...
        <div class="mt-4 space-y-2">
          {#each customerSegments as item}
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
                <div class="w-3 h-3 rounded-full" class:bg-primary={item.color === 'primary'} class:bg-secondary={item.color === 'secondary'} class:bg-accent={item.color === 'accent'}></div>
                <span>{item.name}</span>
              </div>
        ...
        <div class="mt-4 grid grid-cols-4 gap-2 text-center">
          {#each hourlyTraffic.labels as label, i}
            <div>
              ...
              <div class="text-xs font-medium mt-1">{hourlyTraffic.data[i]}</div>
            </div>
          {/each}
        </div>
      </div>
          ...
          {#each deviceTraffic as item}
            <div class="space-y-1">
              ...
              <div class="w-full bg-base-200 rounded-full h-1.5">
                <div class="h-1.5 rounded-full" class:bg-primary={item.color === 'primary'} class:bg-secondary={item.color === 'secondary'} class:bg-accent={item.color === 'accent'} style="width: {item.percentage}%"></div>
              </div>
            </div>
          {/each}
        </div>
