<script lang="ts">
	...
	function getTimeDifference(
		...
		const actualMinutes = Math.floor(
			(new Date(endTime || new Date()).getTime() - new Date(startTime).getTime()) / 1000 / 60
		);
...
<div class="card bg-base-100 p-6 shadow-xl">
	<h3 class="mb-4 text-lg font-semibold">工程ステップ詳細</h3>
		...
		{#each processSteps as step, index (step.id)}
			...
			<div class="flex gap-4" animate:flip={{ duration: 300 }}>
				...
				<div class="flex flex-col items-center">
					<div
						class="flex h-12 w-12 items-center justify-center rounded-full font-bold transition-all duration-300"
					...
					{#if index < processSteps.length - 1}
						<div
							class="mt-1 w-0.5 flex-1 transition-all duration-300"
							class:bg-success={stepData.status === 'completed'}
							class:bg-primary={stepData.status === 'in_progress'}
							class:bg-base-300={stepData.status === 'pending'}
				...
				<div class="flex-1">
					<!-- ステップヘッダー -->
					<div
						class="collapse-arrow bg-base-200 collapse mb-2"
						class:bg-success={stepData.status === 'completed'}
						class:bg-primary={stepData.status === 'in_progress'}
					...
					>
						...
						<div class="collapse-title">
							<div class="flex items-center justify-between">
								<div>
									<h4 class="text-lg font-bold">{step.name}</h4>
							...
							<div class="grid grid-cols-2 gap-4 pt-4">
										...
										<div>
											...
											<div class="flex items-center gap-2">
												<progress
													...
													class:progress-warning={timeDiff.percentage > 100 &&
														timeDiff.percentage <= 120}
								...
								<div class="space-y-3">
									...
									<div class="flex gap-2">
										...
										<button class="btn btn-sm btn-warning" onclick={() => onAlertRegister(step.id)}>
											<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
												<path
													stroke-linecap="round"
													stroke-linejoin="round"
	...
	<div class="stats mt-6 w-full shadow">
		...
		<div class="stat">
			...
			<div class="stat-desc">
				標準: {Math.floor(processSteps.reduce((acc, s) => acc + s.duration, 0) / 60)}h {processSteps.reduce(
			...
			<div class="stat-figure text-accent">
				<div
					...
					style="--value:{Math.round(
						(progress.filter((p) => p.status === 'completed').length / processSteps.length) * 100
					)};"
				>
					{Math.round(
						(progress.filter((p) => p.status === 'completed').length / processSteps.length) * 100
