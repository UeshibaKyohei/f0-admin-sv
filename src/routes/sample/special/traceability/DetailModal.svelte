...
{#if data}
	<dialog class="modal modal-open" transition:fade={{ duration: 200 }}>
		<div class="modal-box max-w-4xl">
			<h3 class="mb-4 text-lg font-bold">{modalTitle}</h3>
							...
							<div class="grid grid-cols-2 gap-4">
								...
								<div>
									...
									<div class="input input-bordered flex items-center">
										<span class="text-error font-semibold">{data.value}</span>
							...
							<div class="grid grid-cols-2 gap-4">
								...
								<div>
									...
									<div class="flex items-center">
										...
										>
											{statusDefinitions[data.status as keyof typeof statusDefinitions]?.label ||
												data.status}
										</span>
									</div>
								</div>
					...
					<div class="space-y-3">
						...
						{:else}
							{#each relatedLogs as log}
								<div
									class="chat"
									class:chat-start={log.type === 'system'}
									class:chat-end={log.type !== 'system'}
								>
									<div class="chat-image avatar">
										<div class="bg-base-300 h-10 w-10 rounded-full">
									...
									<div class="chat-header">
										{log.operator}
										<time class="ml-2 text-xs opacity-50">{formatDateTime(log.timestamp)}</time>
									</div>
									<div
										class="chat-bubble"
					...
					<div class="space-y-4">
						<div class="form-control">
							...
							<div class="flex gap-2">
								...
								></textarea>
								<button class="btn btn-primary" onclick={addComment} disabled={!newComment.trim()}>
									送信
								</button>
							</div>
						</div>
						...
						<div class="stats w-full shadow">
							...
							<div class="stat">
								...
								<div class="stat-value">
									{data.resolved
										? Math.round(
												(new Date(data.resolvedAt).getTime() - new Date(data.timestamp).getTime()) /
													60000
											)
										: Math.round(
							...
							<div class="card-body">
								...
								<div class="flex h-40 items-end gap-2">
									{#each Array(10) as _, i}
										<div
											class="bg-primary flex-1 rounded-t"
