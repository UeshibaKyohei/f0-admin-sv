...
<div class="card bg-base-100 w-full shadow-xl">
	<div class="card-body">
		<h3 class="mb-4 text-lg font-semibold">工程タイムライン</h3>
			...
			<div class="px-8 py-4">
				...
				{#if primaryColor && successColor && baseColor300 && baseColor200}
					<svg width={totalWidth} height={svgHeight} class="overflow-visible">
						...
						{#each processSteps as step, index}
							...
							>
								<!-- ホバー領域用の透明な円 -->
								<circle {cx} cy={circleY} r={circleRadius + 10} fill="transparent" />
								...
								<circle
									...
									cy={circleY}
									r={circleRadius + 4}
									fill={stepData.status === 'completed'
										? successColor
								...
								{#if stepData.status === 'in_progress'}
									...
									>
										<animate
											attributeName="r"
											values="{circleRadius + 8};{circleRadius + 20};{circleRadius + 8}"
											dur="2s"
											repeatCount="indefinite"
										/>
										<animate
				...
				<div class="flex" style="width: {totalWidth}px;">
					{#each processSteps as step, index}
						...
						<div class="text-center" style="width: {stepWidth}px;">
							<h4 class="mt-4 text-sm font-semibold">{step.name}</h4>
							<p class="mt-1 text-xs opacity-70">{step.department}</p>
				...
				<div class="relative">
					{#each processSteps as step, index}
						...
						{#if hoveredStep === step.id}
							<div
								class="bg-base-100 pointer-events-none absolute z-50 min-w-[200px] rounded-lg p-4 shadow-xl"
							...
							>
								<div class="space-y-2 text-sm">
									...
									<div class="flex justify-between">
										...
										>
											{statusDefinitions[stepData.status as keyof typeof statusDefinitions]
				...
				<div class="mt-12">
					...
					<div class="bg-base-300 h-4 w-full overflow-hidden rounded-full">
						<div
							class="from-primary to-success flex h-full items-center justify-end bg-gradient-to-r pr-2 transition-all duration-1000 ease-out"
							style="width: {(progress.filter((p) => p.status === 'completed').length /
								processSteps.length) *
								100}%"
						>
