...
// 待機中の問い合わせ（キューから自動計算）
export const waitingInquiries = derived(inquiryQueue, ($queue) =>
	$queue.filter((inq) => inq.status === 'waiting')
);
...
export function assignInquiry(inquiryId, operatorId, force = false) {
	...
	// 休憩中やオフラインの場合は割り当て不可
	if (
		(currentOperatorData.status === 'break' || currentOperatorData.status === 'offline') &&
		!force
	) {
