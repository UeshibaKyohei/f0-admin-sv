<script>
	...
	async function testEscalation() {
		...
		const targetOperator = $operators.find(
			(op) => op.id !== $currentOperator?.id && op.skills.includes('エスカレーション対応')
		);
		if (!targetOperator) {
	...
	async function fillToCapacity() {
		...
		const currentCount = $activeChats.filter(
			(chat) => chat.assignedTo === selectedOperatorId
		).length;
...
<div class="fixed right-4 bottom-4 z-50">
	<div class="tooltip tooltip-left" data-tip="テスト・デバッグ機能">
		<button class="btn btn-circle btn-warning shadow-lg" onclick={() => (showPanel = !showPanel)}>
			<svg
...
{#if showPanel}
	<div
		class="bg-base-100 border-base-300 fixed right-4 bottom-20 z-50 w-96 rounded-lg border shadow-xl"
	>
		<!-- ヘッダー -->
		<div class="border-base-300 bg-warning/10 flex items-center justify-between border-b p-4">
			<div>
				<h3 class="text-lg font-bold">テスト・デバッグパネル</h3>
				<p class="text-base-content/60 text-xs">開発・デモ用の機能です</p>
			</div>
			<button class="btn btn-ghost btn-sm btn-circle" onclick={() => (showPanel = false)}>
				✕
			</button>
		</div>
		...
		<div class="border-base-300 bg-base-200 border-b p-4">
			<div class="space-y-2">
				...
				<label class="label cursor-pointer">
					...
					<input type="checkbox" class="toggle toggle-sm" bind:checked={$debugSettings.showStats} />
				</label>
			</div>
		</div>
		...
		<div class="max-h-[60vh] overflow-y-auto p-4">
			...
			<div class="bg-base-200 mb-4 rounded p-3">
				<div class="mb-2 text-sm font-medium">現在のオペレーター</div>
				<select
			...
			<div class="space-y-2">
				...
				<button class="btn btn-sm btn-primary w-full" onclick={generateRandomInquiry}>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						class="h-4 w-4"
						fill="none"
				...
				<button class="btn btn-sm btn-secondary w-full" onclick={testAssignment}>
					他のオペレーターに割り当て依頼
				</button>

				<button class="btn btn-sm btn-secondary w-full" onclick={fillToCapacity}>
					選択中のオペレーターを容量まで埋める
				</button>

				<div class="divider text-sm">エスカレーション</div>
				<button class="btn btn-sm btn-warning w-full" onclick={testEscalation}>
					エスカレーションをテスト
				</button>

				<div class="divider text-sm">通知処理</div>
				<button class="btn btn-sm btn-info w-full" onclick={processNotifications}>
					通知を自動処理
				</button>
			...
			{#if $debugSettings.showStats}
				<div class="bg-base-200 mt-4 rounded p-3 text-xs">
					<div class="mb-1 font-medium">システム統計</div>
