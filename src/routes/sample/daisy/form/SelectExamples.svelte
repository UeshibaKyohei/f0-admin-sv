<script>
	...
	const skills = [
		'JavaScript', 'TypeScript', 'React', 'Svelte', 'Vue', 'Angular',
		'Node.js', 'Python', 'PHP', 'Java', 'C#', 'Go', 'Rust',
		'CSS', 'HTML', 'Tailwind CSS', 'Bootstrap', 'SASS',
	...
	<div class="border rounded-lg p-6 bg-base-50">
		...
		<div class="grid md:grid-cols-2 gap-6">
			...
			<div class="mockup-code text-sm">
				<pre><code>&lt;select class="select" bind:value={'{selectedColor}'}&gt;
  &lt;option disabled selected&gt;Pick a color&lt;/option&gt;
  &lt;option value="red"&gt;Crimson&lt;/option&gt;
  &lt;option value="orange"&gt;Amber&lt;/option&gt;
  &lt;option value="purple"&gt;Velvet&lt;/option&gt;
...
&lt;/select&gt;</code></pre>
	...
	<div class="border rounded-lg p-6 bg-base-50">
		...
		<div class="grid md:grid-cols-2 gap-6">
			...
			<div class="mockup-code text-sm">
				<pre><code>&lt;select class="select select-xs"&gt;
  &lt;option disabled selected&gt;Extra Small&lt;/option&gt;
  &lt;option&gt;XS Apple&lt;/option&gt;
&lt;/select&gt;
...
&lt;/select&gt;</code></pre>
	...
	<div class="border rounded-lg p-6 bg-base-50">
		...
		<div class="grid md:grid-cols-2 gap-6">
			...
			<div class="mockup-code text-sm">
				<pre><code>&lt;select class="select select-primary"&gt;
  &lt;option disabled selected&gt;Primary&lt;/option&gt;
  &lt;option&gt;Option 1&lt;/option&gt;
&lt;/select&gt;
		...
		<div class="grid md:grid-cols-2 gap-6">
			<div class="space-y-4">
				<fieldset class="fieldset bg-base-200 border-base-300 rounded-box border p-4">
					...
					<span class="label text-sm text-base-content/70">Choose your preferred browser</span>
				</fieldset>
				...
				<fieldset class="fieldset bg-base-200 border-base-300 rounded-box border p-4">
					...
					<span class="label text-sm text-base-content/70">Selected: {selectedFramework}</span>
				</fieldset>
			</div>
			<div class="mockup-code text-sm">
		...
		<div class="grid md:grid-cols-2 gap-6">
			<div class="space-y-4">
					...
					<div class="dropdown dropdown-bottom w-full">
						<label tabindex="0" class="btn btn-outline w-full justify-between">
							{#if selectedCountries.size > 0}
								{[...selectedCountries].slice(0, 2).join(', ')}{selectedCountries.size > 2 ? ` +${selectedCountries.size - 2} more` : ''}
							{:else}
								Select countries...
							{/if}
						...
						<ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-full max-h-60 overflow-auto">
							{#each countries as country}
								<li>
									<label class="label cursor-pointer justify-start gap-3 hover:bg-base-200 rounded">
										<input 
											type="checkbox" 
				...
				<div class="form-control">
					...
					<div class="dropdown dropdown-bottom w-full">
						<label tabindex="0" class="input input-bordered min-h-12 flex flex-wrap gap-1 items-center cursor-pointer">
							{#each [...selectedSkills] as skill}
								<div class="badge badge-primary gap-1">
									{skill}
									<button class="btn btn-circle btn-ghost btn-xs" onclick={() => toggleSkill(skill)}>
						...
						<ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-full max-h-48 overflow-auto">
				...
				<div class="form-control">
					...
					<label class="label">
						<span class="label-text-alt text-warning">❌ Hard to use - requires Ctrl/Cmd + click</span>
					</label>
				</div>
			</div>
			<div class="mockup-code text-sm">
...
&lt;/div&gt;</code></pre>
	...
	<div class="border rounded-lg p-6 bg-base-50">
		...
		<div class="grid md:grid-cols-2 gap-6">
			...
			<div class="mockup-code text-sm">
				<pre><code>&lt;select class="select select-accent" bind:value={'{selectedPriority}'}&gt;
  &lt;optgroup label="Critical"&gt;
    &lt;option value="urgent"&gt;Urgent&lt;/option&gt;
    &lt;option value="high"&gt;High Priority&lt;/option&gt;
  &lt;/optgroup&gt;
