...
<div class="space-y-8">
		...
		<div class="card-body">
			<div class="mb-4 flex items-start justify-between">
				<div>
					...
					<p class="text-base-content/70 mt-1 text-sm">日付選択と時刻選択を組み合わせた実装例</p>
				</div>
				<button
				...
				<div class="form-control">
					...
					<div class="flex gap-2">
						<input
							...
							readonly
						/>
						<select class="select select-bordered w-24" bind:value={selectedHour}>
							{#each Array(24) as _, i}
								<option value={i}>{String(i).padStart(2, '0')}時</option>
							{/each}
						</select>
						<select class="select select-bordered w-24" bind:value={selectedMinute}>
							{#each [0, 15, 30, 45] as minute}
								<option value={minute}>{String(minute).padStart(2, '0')}分</option>
							{/each}
						</select>
		...
		<div class="card-body">
			<div class="mb-4 flex items-start justify-between">
				<div>
					...
					<p class="text-base-content/70 mt-1 text-sm">複数の日付を選択・管理できる実装例</p>
				</div>
				<button
					class="btn btn-sm btn-ghost"
			...
			<div class="bg-base-200 mb-4 rounded-lg p-6">
				<div class="form-control">
					...
					{#if selectedDates.length > 0}
						<div class="mt-4">
							<p class="mb-2 text-sm font-semibold">選択された日付:</p>
		...
		<div class="card-body">
			<div class="mb-4 flex items-start justify-between">
				<div>
					...
					<p class="text-base-content/70 mt-1 text-sm">常に表示されるカレンダーの実装例</p>
				</div>
				<button class="btn btn-sm btn-ghost" onclick={() => copyToClipboard(inlineCode, 'inline')}>
	...
	<div class="card bg-base-100 shadow-xl">
		<div class="card-body">
			<div class="mb-4 flex items-start justify-between">
				...
				<button class="btn btn-sm btn-ghost" onclick={() => copyToClipboard(modalCode, 'modal')}>
			...
			<dialog id="date_modal" class="modal">
				<div class="modal-box">
					<h3 class="mb-4 text-lg font-bold">日付を選択してください</h3>
			...
			<div class="drawer drawer-end">
				...
				<div class="drawer-side z-50">
					...
					<div class="bg-base-100 min-h-full w-80 p-6">
						<h3 class="mb-4 text-lg font-bold">日付を選択</h3>
						...
						<div class="mt-6">
							<label for="date-drawer" class="btn btn-primary btn-block"> 確定 </label>
						</div>
					</div>
				</div>
			</div>
	...
	<div class="card bg-primary/10 border-primary/20 border-2">
		<div class="card-body">
			<h3 class="text-primary flex items-center gap-2 font-bold">
				<svg
					xmlns="http://www.w3.org/2000/svg"
					class="h-6 w-6"
					fill="none"
