...
<div class="space-y-8">
	...
	<div class="text-center">
		<h2 class="text-primary mb-2 text-3xl font-bold">実用的なユースケース</h2>
							...
							<div class="grid gap-6 md:grid-cols-2">
								...
								<div>
									...
									<div bind:this={reservationContainer} class="reservation-calendar">
										<input
											type="text"
											class="input input-bordered pika-single w-full"
											bind:this={reservationInput}
											value={reservationData.date}
											placeholder="日付を選択してください"
											readonly
									...
									<div class="mt-3 flex gap-4 text-sm">
										<span class="flex items-center gap-1">
											<span class="bg-error h-3 w-3 rounded"></span>
											満席
										</span>
										<span class="flex items-center gap-1">
											<span class="bg-warning h-3 w-3 rounded"></span>
											残りわずか
										</span>
									</div>
								</div>
									...
									{#if reservationData.date}
										<div>
											...
											<div class="grid grid-cols-3 gap-2">
												{#each Object.entries(availableSlots()) as [time, slots]}
													<button
														class="btn btn-sm {reservationData.time === time
															? 'btn-primary'
															: 'btn-outline'} 
                                   {slots === 0 ? 'btn-disabled' : ''}"
														onclick={() => (reservationData.time = time)}
									...
									<div class="grid grid-cols-2 gap-4">
										<div class="form-control">
											...
											<select
												...
												bind:value={reservationData.guestCount}
											>
												{#each Array(8) as _, i}
													<option value={i + 1}>{i + 1}名</option>
												{/each}
									...
									<div class="form-control">
										...
										<div class="input-group">
											<input
												...
												step="0.5"
											/>
											<span class="bg-base-300 flex items-center px-4">時間</span>
										</div>
									</div>
								</div>
							...
							<div class="stats mb-6 shadow">
								<div class="stat">
									...
									<div class="stat-value text-primary">
										{leaveData.remainingDays - leaveData.usedDays}日
									</div>
									<div class="stat-desc">
										今年度：使用済み {leaveData.usedDays}日 / 付与 {leaveData.remainingDays}日
									</div>
								</div>
							</div>
							...
							<div class="space-y-4">
								...
								<div class="form-control">
									...
									<div class="grid grid-cols-2 gap-2 md:grid-cols-4">
										<button
											class="btn btn-sm {leaveData.leaveType === 'paid'
												? 'btn-primary'
												: 'btn-outline'}"
										...
										<button
											class="btn btn-sm {leaveData.leaveType === 'sick'
												? 'btn-warning'
												: 'btn-outline'}"
										...
										<button
											class="btn btn-sm {leaveData.leaveType === 'special'
												? 'btn-info'
												: 'btn-outline'}"
										...
										<button
											class="btn btn-sm {leaveData.leaveType === 'unpaid'
												? 'btn-neutral'
												: 'btn-outline'}"
								...
								{#if calculateBusinessDays > 0}
									{@const hasEnoughDays =
										...
										calculateBusinessDays <= 20 - leaveData.usedDays}
									<div class="alert {hasEnoughDays ? 'alert-info' : 'alert-error'}">
										<svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path
												stroke-linecap="round"
	...
	<div class="card bg-primary/10 border-primary/20 border-2">
		<div class="card-body">
			...
			<div class="grid gap-4 md:grid-cols-2">
				<div>
					<h4 class="mb-2 font-bold">UI/UXの工夫</h4>
				...
				<div>
					<h4 class="mb-2 font-bold">実用的な機能</h4>
					<ul class="space-y-1 text-sm">
