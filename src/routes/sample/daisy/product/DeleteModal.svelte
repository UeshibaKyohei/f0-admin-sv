<script>
	...
	let { products = [], onConfirm, onClose } = $props();
...
<dialog bind:this={modalRef} class="modal modal-bottom sm:modal-middle">
	<div class="modal-box">
		...
		{:else}
			...
			<div class="stats stats-vertical lg:stats-horizontal mb-4 w-full shadow">
				<div class="stat">
			...
			<details class="collapse-arrow bg-base-200 collapse mb-4">
				<summary class="collapse-title text-sm font-medium"> 削除される商品一覧 </summary>
		...
		{#if products.length > 5}
			<div class="form-control mb-4">
				<label class="label">
					<span class="label-text text-sm"> 確認のため「削除」と入力してください </span>
				</label>
				<input
					type="text"
					placeholder="削除"
		...
		<div class="modal-action">
			<button class="btn btn-ghost" onclick={closeModal} disabled={isDeleting}> キャンセル </button>
			<button
				id="confirm-delete"
				class="btn btn-error"
