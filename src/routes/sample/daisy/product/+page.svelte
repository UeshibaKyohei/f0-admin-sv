<script>
	...
	function exportData() {
		...
		const csv = [
			...
			...exportProducts.map((p) => [p.id, p.name, p.sku, p.category, p.price, p.stock, p.status])
		]
			.map((row) => row.join(','))
			.join('\n');
	...
	<div class="mb-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
		...
		<div class="flex flex-wrap gap-2">
			...
			<div class="tooltip tooltip-bottom" data-tip="CSVエクスポート">
				<button class="btn btn-ghost btn-square" onclick={exportData}>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						class="h-5 w-5"
						fill="none"
		...
		{#if viewMode === 'pagination' && totalPages > 1}
			<div class="mt-6 flex justify-center">
				<div class="join">
					...
					<button class="join-item btn" disabled={currentPage === 1} onclick={() => currentPage--}>
						‹
					</button>
