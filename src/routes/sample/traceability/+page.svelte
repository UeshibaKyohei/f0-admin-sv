<script lang="ts">
  ...
  import { batches, batchProgress, processSteps, anomalies as mockAnomalies, workLogs as mockWorkLogs } from './mockData.js';
  ...
  const currentProgress = $derived(
    batchProgress.find(p => p.batchId === selectedBatch?.id)
  );
...
<div class="min-h-screen">
  ...
  <div class="flex gap-4">
    <!-- サイドバー -->
    <div class="w-80 bg-base-100 shadow-xl rounded-lg">
      <div class="p-4">
        <h2 class="text-lg font-semibold mb-4">バッチ一覧</h2>
        ...
        <!-- バッチリスト -->
        <div class="overflow-y-auto max-h-[600px] p-1">
          <div class="space-y-2">
            {#each filteredBatches as batch}
              <div 
                class="card bg-base-200 cursor-pointer hover:shadow-md transition-shadow"
                ...
                <div class="text-xs mt-2">
                  ...
                  <div class="flex justify-between">
                    ...
                    <span class="badge badge-xs" 
                      ...
                      {batch.priority === 'high' ? '高' : 
                       batch.priority === 'normal' ? '中' : '低'}
                    </span>
                  </div>
                </div>
              </div>
    ...
    <div class="flex-1 overflow-hidden">
      {#if selectedBatch}
          ...
          <div class="card-body">
            <div class="flex justify-between items-center gap-4">
              <div class="flex-1">
                ...
                <p class="text-sm opacity-70">バッチID: {selectedBatch.id} | 注文番号: {selectedBatch.orderNumber}</p>
              </div>
              <div class="flex items-center gap-4">
                ...
                <div class="stat">
                  ...
                  <div class="stat-value text-primary">
                    {currentProgress ? Math.round((currentProgress.progress.filter(p => p.status === 'completed').length / processSteps.length) * 100) : 0}%
                  </div>
                </div>
                <div class="stat">
                  <div class="stat-title">残り工程</div>
                  <div class="stat-value text-secondary">
                    {currentProgress ? processSteps.length - currentProgress.progress.filter(p => p.status === 'completed').length : processSteps.length}
                  </div>
                </div>
              </div>
              </div>
        ...
        <div class="tabs tabs-boxed mb-6">
          <a class="tab" class:tab-active={selectedView === 'timeline'} 
             onclick={() => selectedView = 'timeline'}>
            横型タイムライン
          </a>
          <a class="tab" class:tab-active={selectedView === 'steps'} 
             onclick={() => selectedView = 'steps'}>
            縦型ステップ
          </a>
          <a class="tab" class:tab-active={selectedView === 'gantt'} 
