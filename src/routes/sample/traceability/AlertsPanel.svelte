		...
		<div class="mb-4 flex items-center justify-between">
			...
			<div class="flex gap-2">
				...
				<label class="label cursor-pointer gap-2">
					<input type="checkbox" class="toggle toggle-sm" bind:checked={showResolved} />
		...
		{:else}
			<div class="max-h-96 space-y-3 overflow-y-auto">
				{#each processedAlerts as alert (alert.id)}
					<div
						class="alert relative cursor-pointer transition-all duration-200"
					...
					>
						<div class="flex-1">
							<div class="flex items-start gap-3">
								...
								<div class="flex-1">
									...
									{#if alert.resolved}
										<div class="bg-base-200 mt-2 rounded p-2 text-sm">
											<p class="text-base-content font-semibold">対応内容:</p>
								...
								<div class="flex flex-col items-end gap-2">
									{#if !alert.resolved}
										<div class="badge badge-error badge-outline animate-pulse">要対応</div>
									{/if}
								</div>
							</div>
						</div>
						...
						{#if !alert.resolved}
							<button
								class="btn btn-sm btn-circle btn-warning absolute right-2 bottom-2"
								onclick={(e) => {
									e.stopPropagation();
									onAlertEdit(alert);
								}}
