...
export const waitingInquiries = derived(
  inquiryQueue,
  $inquiryQueue => $inquiryQueue.filter(inq => 
    inq.status === 'waiting' && !inq.assignedTo
  )
);
...
export const operatorLoad = derived(
  ...
  ([$operators, $activeChats]) => {
    return $operators.map(op => {
      const currentChats = $activeChats.filter(
        chat => chat.assignedTo === op.id
      );
      ...
      return {
        ...
        canTakeMore: currentChatCount < op.maxConcurrent && 
                     (op.status === 'available' || op.status === 'busy')
      };
    });
  }
);
  ...
  try {
    ...
    if (result.success) {
      ...
      inquiryQueue.update(queue => 
        queue.filter(inq => inq.id !== inquiryId)
      );
...
export function getCurrentOperatorCapacity(operatorId = null) {
  ...
  const activeCount = get(activeChats).filter(
    chat => chat.assignedTo === opId
  ).length;
  
  return {
    current: activeCount,
    max: operator.maxConcurrent,
    canTakeMore: activeCount < operator.maxConcurrent && 
...
export async function acceptEscalation(chatId) {
  try {
    ...
    if (!pending || pending.type !== 'escalation') throw new Error('承認待ちのエスカレーションが見つかりません');
    
    // システムメッセージを追加
    await sendMessage(chatId, `チャットがエスカレーションされました。\n理由: ${pending.reason}\n新しい担当者: ${get(operators).find(op => op.id === pending.targetOperatorId)?.name}`, 'system');
    
    // チャットの担当者を変更
    activeChats.update(chats => 
      chats.map(c => 
        c.id === chatId 
          ? { ...c, assignedTo: pending.targetOperatorId, status: 'active' }
          : c
      )
    );
