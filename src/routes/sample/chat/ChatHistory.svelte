<script>
  ...
  const allArchivedChats = $derived.by(() => {
    ...
    return allChats.sort((a, b) => new Date(b.endTime || b.endDate) - new Date(a.endTime || a.endDate));
  });
  ...
  const filteredChats = $derived.by(() => {
    ...
    if (filterType === 'date') {
      chats = chats.filter(chat => 
        (chat.endTime || chat.endDate || '').startsWith(selectedDate)
      );
    }
...
<div class="grid grid-cols-1 lg:grid-cols-[320px_320px_1fr] h-full w-full">
  <!-- フィルターサイドバー -->
  <div class="hidden lg:flex bg-base-100 border-r border-base-300 overflow-y-auto flex-col">
    <!-- スコープ切り替え -->
    ...
    <div class="p-4 flex-1 overflow-y-auto">
      <h3 class="text-lg font-bold mb-4">フィルター</h3>
  ...
  <!-- 履歴リスト（中央カラム） -->
  <div class="bg-base-100 lg:border-r border-base-300 overflow-y-auto">
    <div class="p-4">
      ...
      <div class="space-y-2">
        {#each filteredChats as chat}
          <button
            class={`w-full text-left card bg-base-200 hover:bg-base-300 transition-colors ${
          ...
          >
            <div class="card-body p-3">
              <div class="flex items-center justify-between mb-1">
                <h3 class="font-medium text-sm">{chat.customerName}</h3>
                <div class="flex items-center gap-1">
                  {#if chat.customerTier}
                    <div class={`badge badge-xs ${
                      chat.customerTier === 'ゴールド' ? 'badge-warning' :
                      chat.customerTier === 'シルバー' ? 'badge-ghost' :
                      'badge-accent'
                    }`}>
                      {chat.customerTier}
                    </div>
                  {/if}
                  <div class={`badge badge-xs ${
                    chat.resolution === 'resolved' ? 'badge-success' :
                    chat.resolution === 'escalated' ? 'badge-error' :
                    'badge-warning'
                  }`}>
                    {chat.resolution === 'resolved' ? '解決' :
                     chat.resolution === 'escalated' ? 'エスカ' :
                     '未解決'}
                  </div>
                </div>
              </div>
              ...
              <div class="text-xs text-base-content/60 mb-1">
                {new Date(chat.endTime || chat.endDate).toLocaleDateString('ja-JP')} {new Date(chat.endTime || chat.endDate).toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' })}
                {#if viewScope === 'all'}
                  <br/>対応: {chat.operatorName}
                {/if}
              </div>
              
              <p class="text-sm line-clamp-2">{chat.subject}</p>
  ...
  <!-- 詳細表示（右カラム） -->
  <div class="bg-base-100 overflow-hidden flex flex-col">
    {#if selectedChat}
      <div class="p-6 border-b border-base-300">
        <div>
          <div class="flex items-start justify-between mb-4">
            <div class="flex-1">
              <h2 class="text-2xl font-bold mb-2">{selectedChat.customerName}</h2>
              <div class="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-base-content/60">
                ...
                <span class={`flex items-center gap-1 font-medium ${
                  selectedChat.resolution === 'resolved' ? 'text-success' :
                  selectedChat.resolution === 'escalated' ? 'text-error' :
                  'text-warning'
                }`}>
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={
                      selectedChat.resolution === 'resolved' ? "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" :
                      selectedChat.resolution === 'escalated' ? "M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" :
                      "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"
                    } />
                  </svg>
                  {selectedChat.resolution === 'resolved' ? '解決済み' :
                   selectedChat.resolution === 'escalated' ? 'エスカレーション' :
                   '未解決'}
                </span>
              </div>
            </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div class="bg-base-200 rounded-lg p-4">
              <div class="text-sm text-base-content/60 mb-2 font-medium">対応内容の要約</div>
              <p class="text-sm leading-relaxed">{selectedChat.summary}</p>
            </div>
            
            <div class="bg-base-200 rounded-lg p-4">
              <div class="text-sm text-base-content/60 mb-2 font-medium">対応メトリクス</div>
              <div class="space-y-2">
                <div class="flex justify-between items-center">
                  <span class="text-sm text-base-content/70">初回応答時間</span>
                  <span class="text-sm font-medium">{formatTime(selectedChat.responseTime)}</span>
                </div>
                <div class="flex justify-between items-center">
                  <span class="text-sm text-base-content/70">解決時間</span>
                  <span class="text-sm font-medium">{formatTime(selectedChat.resolutionTime)}</span>
                </div>
                {#if selectedChat.satisfactionScore}
                  <div class="flex justify-between items-center">
                    <span class="text-sm text-base-content/70">顧客満足度</span>
                    <span class="text-sm">
                      {#each Array(5) as _, i}
                        <span class={i < selectedChat.satisfactionScore ? 'text-warning' : 'text-base-300'}>
                          ★
                        </span>
                      {/each}
                    </span>
      ...
      <div class="flex-1 overflow-y-auto p-6">
        <h3 class="text-lg font-semibold mb-4">会話履歴</h3>
        <div class="space-y-3">
          {#each selectedChat.messages as message}
            ...
            {:else}
              <div class={`flex ${message.type === 'agent' ? 'justify-end' : 'justify-start'} mb-4`}>
                <div class={`max-w-[80%] ${message.type === 'agent' ? 'text-right' : 'text-left'}`}>
                  <div class="text-xs text-base-content/60 mb-1">
                    {message.type === 'agent' ? selectedChat.operatorName : selectedChat.customerName}
    ...
    {:else}
      <div class="flex-1 flex items-center justify-center text-base-content/60">
        <div class="text-center">
          ...
          <p class="text-lg font-medium mb-2">履歴を選択してください</p>
          <p class="text-sm">左側のリストから対応履歴を選択すると<br/>詳細な会話内容が表示されます</p>
        </div>
      </div>
    {/if}
  </div>
