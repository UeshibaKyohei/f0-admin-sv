<script>
	...
	// 現在のチャット情報
	const currentChat = $derived($activeChats.find((chat) => chat.id === $selectedChatId));

	// 顧客情報
	const customer = $derived(currentChat ? $customers[currentChat.customerId] : null);

	// 過去の履歴
	const customerHistory = $derived(customer ? $archivedChats[customer.id] || [] : []);
...
<div class="bg-base-100 border-base-300 w-96 flex-shrink-0 overflow-y-auto border-l">
	{#if customer}
		<div class="space-y-4 p-4">
			...
			<div>
				...
				<div class="space-y-2">
					<div class="flex items-center justify-between">
						<span class="text-base-content/70 text-sm">名前</span>
						<span class="font-medium">{customer.name}</span>
					</div>

					<div class="flex items-center justify-between">
						<span class="text-base-content/70 text-sm">メール</span>
						<a href={`mailto:${customer.email}`} class="link link-primary truncate text-sm">
							{customer.email}
						</a>
					</div>

					<div class="flex items-center justify-between">
						<span class="text-base-content/70 text-sm">電話</span>
						<span class="text-sm">{customer.phone}</span>
					</div>

					<div class="flex items-center justify-between">
						<span class="text-base-content/70 text-sm">会員ランク</span>
						<div
							class={`badge ${
								customer.tier === 'ゴールド'
									? 'badge-warning'
					...
					<div class="flex items-center justify-between">
						<span class="text-base-content/70 text-sm">登録日</span>
						<span class="text-sm">{formatDate(customer.registeredDate)}</span>
					</div>

					{#if customer.previousComplaints > 0}
						<div class="flex items-center justify-between">
							<span class="text-base-content/70 text-sm">過去のクレーム</span>
							<span class="text-error text-sm font-medium">{customer.previousComplaints}件</span>
						</div>
					{/if}
				</div>
			</div>
			...
			{#if customer.lastOrder}
				<div>
					<h4 class="mb-2 font-medium">最新の注文</h4>
								...
								<div class="flex justify-between">
									...
									<div
										class={`badge badge-sm ${
											customer.lastOrder.status === '配送済み'
												? 'badge-success'
												: customer.lastOrder.status === '配送中'
													? 'badge-info'
													: 'badge-warning'
										}`}
									>
								...
								<div class="card-body p-3">
									<div class="text-sm">
										<div class="mb-1 flex items-start justify-between">
											...
											<div
												class={`badge badge-xs ${
													history.resolution === 'resolved'
														? 'badge-success'
														: history.resolution === 'escalated'
															? 'badge-error'
															: 'badge-warning'
												}`}
											>
										...
										<div class="text-base-content/70 mb-1 text-xs">
											{formatRelativeTime(history.date)} | 対応時間: {history.resolutionTime}分
										</div>
										<div class="text-xs">
											{history.summary}
										</div>
										{#if history.satisfactionScore}
											<div class="mt-1">
												...
												{#each Array(5) as _, i}
													<span
														class={i < history.satisfactionScore ? 'text-warning' : 'text-base-300'}
													>
														★
													</span>
												{/each}
			...
			{#if customer.tags && customer.tags.length > 0}
				...
				<!-- タグ -->
				<div>
					<h3 class="mb-3 font-semibold">顧客タグ</h3>
			...
			{#if customer.notes && customer.notes.length > 0}
				...
				<!-- メモ -->
				<div>
					<h3 class="mb-3 font-semibold">顧客メモ</h3>
					<div class="space-y-2">
			...
			{#if customer.recentOrders && customer.recentOrders.length > 1}
				...
				<!-- 最近の注文履歴 -->
				<div>
					<h3 class="mb-3 font-semibold">最近の注文</h3>
					<div class="space-y-2">
			...
			<!-- クイックアクション -->
			<div>
				<h3 class="mb-3 font-semibold">クイックアクション</h3>
				<div class="space-y-2">
