<script>
	...
	const relevantTemplates = $derived.by(() => {
		...
		const categoryTemplates = allTemplates.filter(
			(t) => t.category === currentChat.category || t.category === 'general'
		);
...
{#if currentChat}
	<div class="bg-base-100 flex h-full flex-col">
		<!-- チャットヘッダー -->
		<div class="bg-base-100 border-base-300 flex-shrink-0 border-b">
			<!-- 上段：顧客情報とアクション -->
			<div class="flex items-center justify-between px-4 py-2">
				<div class="flex items-center gap-3">
					<div>
						<div class="flex items-center gap-2">
							<h3 class="text-lg font-semibold">{currentChat.customerName}</h3>
							{#if currentCustomer?.tier}
								<div
									class={`badge badge-sm ${
										currentCustomer.tier === 'ゴールド'
											? 'badge-warning'
											: currentCustomer.tier === 'シルバー'
												? 'badge-ghost'
												: 'badge-accent'
									}`}
								>
						...
						<div class="text-base-content/60 text-sm">
							{inquiryCategories[currentChat.category]?.icon}
							{inquiryCategories[currentChat.category]?.label}
						</div>
					</div>
				</div>
								...
								<a>
									...
									>
										<path
											...
											stroke-linejoin="round"
											stroke-width="2"
											d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m9.032 4.026a9.001 9.001 0 11-9.032-4.026m9.032 4.026A9.001 9.001 0 0112 21m9.032-6.316A9.001 9.001 0 0112 3"
										/>
									</svg>
									転送
								</a>
			...
			<!-- 下段：ステータスと情報 -->
			<div class="border-base-300 flex items-center justify-between border-t px-4 pb-2">
				<div class="flex items-center gap-4 text-sm">
					...
					<div class="flex items-center gap-2">
						...
						<div class="dropdown dropdown-hover">
							<label
								tabindex="0"
								class={`badge ${chatStatuses[currentChat.status || 'active']?.color || 'badge-success'} cursor-pointer`}
							>
								{chatStatuses[currentChat.status || 'active']?.label || '対応中'}
							</label>
							<ul
								tabindex="0"
								class="dropdown-content menu bg-base-100 rounded-box z-10 w-32 p-1 shadow"
							>
								{#each Object.entries(chatStatuses) as [key, status]}
									<li>
										<a onclick={() => changeStatus(key)} class="py-1 text-sm">{status.label}</a>
									</li>
								{/each}
							</ul>
						</div>
					...
					<div class="flex items-center gap-2">
						...
						<div class="dropdown dropdown-hover">
							<label
								tabindex="0"
								class={`badge ${priorities[currentChat.priority || 'normal']?.color || 'badge-primary'} cursor-pointer`}
							>
							...
							>
								{#each Object.entries(priorities) as [key, priority]}
									<li>
										<a onclick={() => changePriority(key)} class="py-1 text-sm">{priority.label}</a>
									</li>
								{/each}
							</ul>
						</div>
		...
		<div class="flex flex-1 flex-col overflow-hidden">
			<div bind:this={messageContainer} class="flex-1 space-y-4 overflow-y-auto p-4">
				...
				{#each chatMessages as message}
					<div
						class={`chat ${message.type === 'agent' ? 'chat-end' : message.type === 'system' ? 'chat-center' : 'chat-start'}`}
					>
						...
						{:else}
							...
							<div
								class={`chat-bubble ${message.type === 'agent' ? 'chat-bubble-primary' : ''} whitespace-pre-wrap`}
							>
								{message.content}
							</div>
						{/if}
			...
			<!-- 入力エリア -->
			<div class="border-base-300 bg-base-100 flex-shrink-0 border-t p-3">
				<!-- クイックアクション -->
				<div class="mb-2 flex items-center justify-between">
					...
					<!-- テンプレートボタン -->
					<button class="btn btn-ghost btn-xs gap-1" onclick={() => (showTemplateModal = true)}>
						<svg
							xmlns="http://www.w3.org/2000/svg"
							class="h-4 w-4"
							fill="none"
				...
				<div class="flex gap-2">
					<textarea
						...
						class="textarea textarea-bordered max-h-32 min-h-[3rem] flex-1"
						rows="1"
					/>
		...
		<!-- 履歴パネル -->
		{#if showHistory}
			<div class="border-base-300 bg-base-100 w-80 overflow-y-auto border-l">
				<div class="p-4">
					<h3 class="mb-3 font-semibold">過去の問い合わせ履歴</h3>
					...
					{:else}
						<p class="text-base-content/60 text-sm">履歴はありません</p>
					{/if}
				</div>
			</div>
		{/if}
	...
	{#if showEscalateModal}
		<div class="modal modal-open">
			<div class="modal-box">
				<h3 class="mb-4 text-lg font-bold">エスカレーション</h3>
				...
				<div class="form-control mb-4">
					...
					<select bind:value={escalateToId} class="select select-bordered">
						...
						{#each availableOperators as op}
							<option value={op.id}>
								{op.name}
								({op.skills.join(', ')}) - {op.status === 'available' ? '対応可能' : '対応中'}
							</option>
						{/each}
					</select>
				</div>
				...
				<div class="modal-action">
					<button class="btn" onclick={() => (showEscalateModal = false)}> キャンセル </button>
					<button
						class="btn btn-error"
	...
	{#if showTemplateModal}
		<div class="modal modal-open">
			<div class="modal-box max-w-2xl">
				<h3 class="mb-4 text-lg font-bold">テンプレートを選択</h3>
						...
						>
							<div class="card-body p-3">
								<div class="flex items-start justify-between">
									<div class="flex-1">
										<h4 class="mb-1 text-sm font-medium">{template.title}</h4>
									...
									<div
										class={`badge badge-sm ${
											template.category === currentChat?.category
												? 'badge-primary'
