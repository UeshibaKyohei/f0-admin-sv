...
<div class="bg-base-100 border-base-300 flex-shrink-0 overflow-x-auto border-b">
	<div class="flex h-12">
		{#each $activeChats as chat}
			{@const isSelected = $selectedChatId === chat.id}
			<div
				class={`
          border-base-300 hover:bg-base-200 flex min-w-[180px] cursor-pointer items-center
          gap-2 border-r px-3 transition-colors
          ${isSelected ? 'bg-base-200 border-b-primary border-b-2' : ''}
        `}
				...
				<div class="min-w-0 flex-1">
					<div class="flex items-center gap-1">
						...
						{#if chat.priority === 'urgent' || chat.priority === 'high'}
							<div
								class={`badge badge-xs ${priorities[chat.priority || 'normal']?.color || 'badge-primary'}`}
							>
								!
							</div>
						{/if}
					...
					<div class="text-base-content/60 flex items-center gap-2 text-xs">
						...
						<span class={`${chatStatuses[chat.status || 'active']?.color || 'badge-success'}`}
							>•</span
						>
					</div>
				</div>
		...
		{#if $activeChats.length === 0}
			<div class="text-base-content/60 flex items-center px-4">対応中のチャットはありません</div>
		{/if}
	</div>
</div>
...
{#if showResolveModal}
	<div class="modal modal-open">
		<div class="modal-box">
			<h3 class="mb-4 text-lg font-bold">チャットを終了</h3>
			...
			<div class="modal-action">
				<button class="btn" onclick={() => (showResolveModal = false)}> キャンセル </button>
				<button class="btn btn-primary" onclick={confirmResolve} disabled={!summary.trim()}>
					終了する
				</button>
			</div>
		</div>
