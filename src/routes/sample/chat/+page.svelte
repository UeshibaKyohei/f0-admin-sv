...
<div class="h-[calc(100vh-3.5rem)] -m-4 md:-m-6 bg-base-200 flex flex-col overflow-hidden">
  <!-- ヘッダー -->
  <header class="bg-base-100 border-b border-base-300 px-4 py-2 flex-shrink-0">
        ...
        {#if $currentOperator}
          ...
          <div class="dropdown dropdown-end">
            <label tabindex="0" class="btn btn-ghost btn-sm gap-2">
              <div class={`w-2 h-2 rounded-full ${
                $currentOperator.status === 'available' ? 'bg-success' :
                $currentOperator.status === 'busy' ? 'bg-warning' :
                $currentOperator.status === 'break' ? 'bg-info' :
                'bg-base-300'
              }`}></div>
              <span class="text-sm">
                {$currentOperator.status === 'available' ? '対応可能' :
                 $currentOperator.status === 'busy' ? '対応中' :
                 $currentOperator.status === 'break' ? '休憩中' :
                 'オフライン'}
              </span>
          ...
          <div class="flex items-center gap-2">
            <div class="text-right">
              ...
              <div class="text-xs text-base-content/60">本日: {$currentOperator.todayHandled}件</div>
            </div>
        ...
        <div class="bg-base-100 border-b border-base-300 px-4 py-3 flex-shrink-0">
          <div class="flex items-center gap-6 overflow-x-auto overflow-y-hidden">
            <div class="flex items-center gap-2 whitespace-nowrap">
              <div class="text-xs text-base-content/60">待機中</div>
              <div class="text-lg font-bold text-warning">{$waitingInquiries.length}</div>
            </div>
            <div class="flex items-center gap-2 whitespace-nowrap">
              <div class="text-xs text-base-content/60">対応中</div>
              <div class="text-lg font-bold text-success">{$activeChats.length}</div>
            </div>
            <div class="flex items-center gap-2 whitespace-nowrap">
              <div class="text-xs text-base-content/60">本日対応</div>
              <div class="text-lg font-bold">{$operators.reduce((sum, op) => sum + op.todayHandled, 0)}</div>
            </div>
            <div class="flex items-center gap-2 whitespace-nowrap">
              <div class="text-xs text-base-content/60">平均応答</div>
            ...
            <div class="flex-1 flex items-center gap-3 justify-end">
              {#each $operatorLoad.slice(0, 5) as op}
                <div class="tooltip tooltip-bottom" data-tip="{op.name}: {op.currentChats.length}/{op.maxConcurrent}件">
                  {#if op.avatar}
                    <div class="avatar">
                      <div class="w-8 rounded-full ring-2 ring-offset-base-100 ring-offset-1 {
                        op.load >= 80 ? 'ring-error' :
                        op.load >= 60 ? 'ring-warning' :
                        'ring-success'
                      }">
                  ...
                  {:else}
                    <div class="avatar avatar-placeholder">
                      <div class="bg-neutral text-neutral-content w-8 rounded-full ring-2 ring-offset-base-100 ring-offset-1 {
                        op.load >= 80 ? 'ring-error' :
                        op.load >= 60 ? 'ring-warning' :
                        'ring-success'
                      }">
            ...
            {#if $selectedChatId}
              ...
              <div class="flex-1 flex overflow-hidden">
                <div class="flex-1 relative">
                  ...
                  {#if innerWidth >= 1024}
                    <button
                      class="absolute right-0 top-1/2 -translate-y-1/2 bg-base-100 border border-l-0 border-base-300 rounded-r-lg p-2 hover:bg-base-200 transition-colors z-10"
            ...
            {:else}
              ...
              <div class="flex-1 flex items-center justify-center bg-base-100">
                <div class="text-center">
                  ...
                  <h2 class="text-xl font-bold mb-2">チャットを選択してください</h2>
